      ...
      return (
        <Collapsible key={item.id} open={isExpanded} onOpenChange={() => toggleExpanded(item.id)}>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" className={cn('w-full justify-start gap-3 h-10 rounded-lg', isCollapsed && 'justify-center px-2')}>
              {itemContent}
            </Button>
          </CollapsibleTrigger>
          ...
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg overflow-hidden">
                <img src="/img/ms-icon-310x310.png" alt="SIRP-NNA" className="h-full w-full object-cover" />
              </div>
              <span className="font-semibold text-sidebar-foreground">SIRP-NNA</span>
            </div>
          )}
          {isMobile && (
            <Button variant="ghost" size="icon" onClick={closeMobileDrawer} className="h-9 w-9">
